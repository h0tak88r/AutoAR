...
def find_wordlist_file(filename):
    """
    Smart path resolution for wordlist files.
    Checks in order:
    1. Wordlists/ directory (relative to script location)
    2. Current directory
    3. Config value (if exists)
    4. Just the filename (for absolute paths)
    """
    # Get script directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # Try Wordlists/ relative to script (go up to project root)
    # Script could be at: python/jwt_tool.py or python/jwt_tool/jwt_tool.py
    wordlists_paths = [
        os.path.join(script_dir, '..', 'Wordlists', filename),  # python/ -> Wordlists/
        os.path.join(script_dir, '..', '..', 'Wordlists', filename),  # python/jwt_tool/ -> Wordlists/
        os.path.join(os.getcwd(), 'Wordlists', filename),
        os.path.join(os.getcwd(), filename),
        filename
    ]
    
    for filepath in wordlists_paths:
        abs_path = os.path.abspath(filepath)
        if os.path.isfile(abs_path):
            return abs_path
    
    # If not found, return original filename (will error later if file doesn't exist)
    return filename
...
def createConfig():
    ...
    # Store filenames - will be resolved when files are opened
...
def scanModePlaybook():
    ...
    if headDict['alg'][:2] == "HS" or headDict['alg'][:2] == "hs":
        ...
        config['argvals']['keyList'] = find_wordlist_file("jwt-common.txt")
    ...
    if headDict['alg'][:2] != "HS" and headDict['alg'][:2] != "hs":
        ...
        with open(find_wordlist_file('jwks-common.txt'), "r", encoding='utf-8', errors='ignore') as jwksLst:
...
def preScan():
    ...
    if config['argvals']['canaryvalue']:
        if config['argvals']['canaryvalue'] not in config['argvals']['rescontent']:
            ...
            if args.yes:
                cprintc("Continuing automatically (--yes flag set)", "yellow")
            else:
...
if __name__ == '__main__':
    ...
    if (os.path.isfile(configFileName)):
        ...
        if 'input' in config:
            ...
            config['input']['commonHeaders'] = find_wordlist_file(config['input'].get('commonHeaders', 'common-headers.txt'))
            config['input']['commonPayloads'] = find_wordlist_file(config['input'].get('commonPayloads', 'common-payloads.txt'))
    else:
