...
func handleApkXScan(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	go func(filename, path string, mitm bool) {
		...
		if err == nil && res != nil && res.ReportDir != "" {
			if err := cleanupResultsDirectory(res.ReportDir); err != nil {
...
func handleApkXScanFromPackage(s *discordgo.Session, i *discordgo.InteractionCreate, pkg string, mitm bool) {
	...
	extraFiles := prepareAPKFilesWithR2Links(res, mitm, &fields, r2URLs, mitmAPKURL)
	...
	// Send Discord message — include any extra file attachments (e.g. MITM APK when R2 is off)
	log.Printf("[DEBUG] Sending Discord message with %d extra file attachments", len(extraFiles))
	var sendErr error
	if len(extraFiles) > 0 {
		var msg *discordgo.Message
		msg, sendErr = s.FollowupMessageCreate(i.Interaction, false, &discordgo.WebhookParams{
			Embeds: []*discordgo.MessageEmbed{embed},
			Files:  extraFiles,
		})
		if sendErr != nil {
			log.Printf("[ERROR] Failed to send Discord message with files: %v", sendErr)
		} else if msg != nil {
			log.Printf("[DEBUG] Successfully sent Discord message with %d files (message ID: %s)", len(extraFiles), msg.ID)
		}
	} else {
		var msg *discordgo.Message
		msg, sendErr = s.FollowupMessageCreate(i.Interaction, false, &discordgo.WebhookParams{
		...
		if sendErr != nil {
			log.Printf("[ERROR] Failed to send Discord message: %v", sendErr)
		} else if msg != nil {
			log.Printf("[DEBUG] Successfully sent Discord message (message ID: %s)", msg.ID)
		} else {
			log.Printf("[WARN] FollowupMessageCreate returned nil message without error")
	...
	if sendErr != nil && i.ChannelID != "" {
		// Fallback: send embed to the channel directly if the interaction timed out
		_, channelErr := s.ChannelMessageSendEmbed(i.ChannelID, embed)
		if channelErr != nil {
			log.Printf("[ERROR] Failed to send fallback channel message: %v", channelErr)
		} else {
...
// Returns any files that should be attached to the Discord message (e.g. MITM APK when R2 is off).
func prepareAPKFilesWithR2Links(res *apkxmod.Result, mitm bool, fields *[]*discordgo.MessageEmbedField, r2URLs map[string]string, mitmAPKURL string) []*discordgo.File {
	var extraFiles []*discordgo.File
	if res == nil {
		return extraFiles
	}
	...
	// Add MITM patched APK — prefer R2 link, fall back to Discord file attachment
	if mitm && res.MITMPatchedAPK != "" {
		apkName := filepath.Base(res.MITMPatchedAPK)
		if mitmAPKURL != "" {
			// Build the field value: include size if local file still exists, otherwise just the link
			fieldValue := fmt.Sprintf("`%s`\n🔗 [Download from R2](%s)", apkName, mitmAPKURL)
			if stat, statErr := os.Stat(res.MITMPatchedAPK); statErr == nil {
				fieldValue = fmt.Sprintf("`%s` (%.2f MB)\n🔗 [Download from R2](%s)",
					apkName, float64(stat.Size())/1024/1024, mitmAPKURL)
			}
			*fields = append(*fields, &discordgo.MessageEmbedField{
			...
			// R2 not configured — attach APK as Discord file (if it still exists locally)
			if data, readErr := os.ReadFile(res.MITMPatchedAPK); readErr == nil {
				extraFiles = append(extraFiles, &discordgo.File{
					Name:        apkName,
				...
				*fields = append(*fields, &discordgo.MessageEmbedField{
					Name:   "🔓 MITM Patched APK",
					Value:  fmt.Sprintf("`%s` — attached above", apkName),
					Inline: false,
				})
				log.Printf("[MITM] Attaching patched APK as Discord file (R2 not configured): %s", res.MITMPatchedAPK)
			} else {
				log.Printf("[WARN] MITM APK not found for attachment: %s — %v", res.MITMPatchedAPK, readErr)
			}
		}
	}
...
func prepareAPKFiles(res *apkxmod.Result) []*discordgo.File {
