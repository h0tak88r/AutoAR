...
async function analyzeIPA(file) {
    ...
    ipaState.findings = { high: [], warning: [], info: [], secure: [] };
    ipaState.groupedFindings = { high: [], warning: [], info: [], secure: [] };
    ipaState.fileContents.clear();
    ipaState.binaryStrings = [];
    ...
    try {
        ...
        if (plistFile) {
            ...
            ipaState.fileContents.set('Info.plist', JSON.stringify(results.plistData, null, 2));
        ...
        for (const [path, entry] of Object.entries(zip.files)) {
            ...
            if (textExts.includes(ext)) {
                try {
                    ...
                    ipaState.fileContents.set(relPath, content);
        ...
        if (results.appInfo.executableName) {
            ...
            if (binFile) {
                ...
                ipaState.binaryStrings = stringData;
        ...
        for (const f of results.findings) {
            ...
            if (ipaState.findings[sev]) ipaState.findings[sev].push(f);
        }

        ipaState.groupedFindings = groupFindingsByRule(results.findings);

        updateProgress(95, 'Calculating score...');
        let score = 100;
        score -= ipaState.groupedFindings.high.length * 12;
        score -= ipaState.groupedFindings.warning.length * 4;
        score += ipaState.groupedFindings.secure.length * 2;
        results.securityScore = Math.max(0, Math.min(100, score));

        ipaState.analysisResults = results;
        log('Analysis complete!', {
            total: results.findings.length,
            high: ipaState.findings.high.length,
            warning: ipaState.findings.warning.length,
            secure: ipaState.findings.secure.length
        });
...
async function loadFileContent(filePath, options = {}) {
    ...
    if (!options.hexOffset && !options.isBinaryReference && ipaState.fileContents.has(cacheKey)) {
        const cached = ipaState.fileContents.get(cacheKey);

        if (typeof cached === 'string' && !cached.startsWith('bplist')) {
            return cached;
        }

        ipaState.fileContents.delete(cacheKey);
    }

    if (!ipaState.zipContent || !ipaState.appPath) return null;
    ...
    if (options.isBinaryReference) {
        ...
        const possiblePaths = [
            ipaState.appPath + execName,
            ipaState.appPath + '/' + execName,
            execName
        ];

        for (const path of possiblePaths) {
            file = ipaState.zipContent.file(path);
            if (file) {
        ...
        if (!file) {
            const allFiles = Object.keys(ipaState.zipContent.files);
            const execFile = allFiles.find(f => f.endsWith('/' + execName) || f === execName);
            if (execFile) {
                file = ipaState.zipContent.file(execFile);
                log('Found executable via search:', execFile);
            }
        }
    } else {
        const fullPath = ipaState.appPath + filePath;
        file = ipaState.zipContent.file(fullPath) || ipaState.zipContent.file(filePath);
    }
            ...
            if (isBinaryPlist) {
                try {
                    ...
                    if (parsed) {
                        ...
                        ipaState.fileContents.set(cacheKey, content);
...
function searchBinaryStrings(query) {
    if (!ipaState.binaryStrings.length) return [];

    const lower = query.toLowerCase();
    return ipaState.binaryStrings
...
function searchFiles(query) {
    if (!ipaState.analysisResults) return [];

    const lower = query.toLowerCase();
    return ipaState.analysisResults.files
...
function exportReport() {
    const results = ipaState.analysisResults;
    ...
    const report = {
        ...
        summary: {
            high: ipaState.findings.high.length,
            warning: ipaState.findings.warning.length,
            info: ipaState.findings.info.length,
            secure: ipaState.findings.secure.length
        },
