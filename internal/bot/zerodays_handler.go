...
func getLiveHosts(domain string, threads int) (string, error) {
	...
	if shouldCollect {
		...
		subCmd := exec.Command(utils.GetAutoarScriptPath(), "subdomains", "get", "-d", domain, "-t", fmt.Sprintf("%d", threads), "-s")
	...
	cmd := exec.Command(utils.GetAutoarScriptPath(), "livehosts", "get", "-d", domain, "-t", fmt.Sprintf("%d", threads), "--silent")
