...
// Note: api.ScanInfo is shared between commands.go and api.go
...
func storeChannelID(scanID, channelID string) {
	api.ChannelsMutex.Lock()
	api.ActiveChannels[scanID] = channelID
	api.ChannelsMutex.Unlock()
}
...
func runScanBackground(scanID, scanType, target string, command []string, s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if i != nil && i.ChannelID != "" {
		...
		if chID := api.GetChannelID(scanID); chID == "" {
	...
	if exists {
		...
		api.ActiveScans[scanID] = &api.ScanInfo{
			ScanID:     scanID,
			Type:       scanType,
			ScanType:   scanType,
			Target:     target,
	...
	if err == nil && status == "completed" && (scanType == "domain_run" || scanType == "subdomain_run") {
		api.IndexWorkflowArtifactsFromR2(scanID, scanType, target)
	}
	...
	if isCancelled {
		...
		if scanType == "domain_run" || scanType == "subdomain_run" {
			...
			if zipURL == "" {
				outputStr := string(output)
				zipURL = utils.ExtractR2ZipURLFromOutput(outputStr)
				if zipURL != "" {
	...
	if shouldCleanup {
		...
		if cleanupPath != "" {
			if err := cleanupResultsDirectory(cleanupPath); err != nil {
...
func sendResultFiles(s *discordgo.Session, i *discordgo.InteractionCreate, scanID, scanType, target string) {
	...
	if scanID != "" {
		...
		if scan, ok := api.ActiveScans[scanID]; ok && scan.ThreadID != "" {
			...
			log.Printf("[DEBUG] No thread found for scan %s in api.ActiveScans", scanID)
		}
		api.ScansMutex.RUnlock()
	} else {
	...
	// Send each result file that exists and track whether anything was actually sent
	filesSent := 0
	...
	// If we expected result files but none existed/had content, tell the user clearly
	if len(resultFiles) > 0 && filesSent == 0 && i != nil {
		scanLabel := scanType
		switch scanType {
		case "github_scan":
			scanLabel = "GitHub Repo Scan"
		case "github_org":
			scanLabel = "GitHub Org Scan"
		case "github_experimental":
			scanLabel = "GitHub Experimental Scan"
		case "githubdepconf_scan", "github_depconf":
			scanLabel = "GitHub DepConfusion Scan"
		}
		embed := &discordgo.MessageEmbed{
			Title:       fmt.Sprintf("✅ %s — No Findings", scanLabel),
			Description: fmt.Sprintf("**Target:** `%s`\n\nThe scan completed successfully but found **no secrets or vulnerabilities**.", target),
			Color:       0x2ecc71,
		}
		if threadID != "" {
			s.ChannelMessageSendEmbed(threadID, embed)
		} else {
...
func handleScanSubdomain(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	command := []string{utils.GetAutoarScriptPath(), "subdomains", "get", "-d", subdomain}
	if verbose {
		command = append(command, "-v")
	}
...
func handleFastLook(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	command := []string{utils.GetAutoarScriptPath(), "fastlook", "run", "-d", domain}
	if verbose {
		command = append(command, "-v")
	}
...
func handleDomainRun(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	skip := skipFFuf
	go api.RunScanInProcess(scanID, "domain_run", domain, func() error {
...
func handleSubdomainRun(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	sub := subdomain
	go api.RunScanInProcess(scanID, "subdomain_run", subdomain, func() error {
