        ...
        const instances = (f.instances || []).slice(0, 200).map((inst, j) => {
            ...
            return html`
              <div class="instance-item">
                ...
                ${inst.match ? raw('<div class="instance-match"><code>' + esc((inst.match || '').slice(0, 500)) + '</code><button class="copy-btn" data-copy="' + escAttr(inst.match || '') + '" title="Copy match" aria-label="Copy match value">[GRID]</button></div>') : ''}
              </div>`;
        }).join('');
        return html`
          <article class="finding-card ${sevCls}" data-severity="${f.severity}" data-finding-id="${idx}">
            <button class="finding-header" aria-expanded="false" aria-controls="finding-body-${idx}">
              ...
              <span class="finding-toggle" aria-hidden="true">[DOWN]</span>
            </button>
...
function renderPager(total) {
    ...
    wrap.innerHTML = `
      <button class="pager-btn" data-pager="first" ${cur === 0 ? 'disabled' : ''}>[PREV] First</button>
      <button class="pager-btn" data-pager="prev"  ${cur === 0 ? 'disabled' : ''}>[LEFT] Prev</button>
      <span class="pager-info">Page ${cur + 1} of ${pages}</span>
      <button class="pager-btn" data-pager="next" ${cur >= pages - 1 ? 'disabled' : ''}>Next [PLAY]</button>
      <button class="pager-btn" data-pager="last" ${cur >= pages - 1 ? 'disabled' : ''}>Last [NEXT]</button>`;
...
function onTreeClick(e) {
    ...
    if (folderHeader) {
        ...
        if (ic) ic.textContent = folder.classList.contains('open') ? '[OPEN]' : '[FOLDER]';
        return;
    }
    ...
    for (const [name, node] of entries) {
        if (node._type === 'dir') {
            ...
            out += '<div class="tree-folder" role="treeitem" aria-expanded="false">' +
                    ...
                    '<span class="folder-chevron" aria-hidden="true">[PLAY]</span>' +
...
function fileIcon(ext, name) {
    const lower = (name || '').toLowerCase();
    if (lower === 'androidmanifest.xml') return '[LIST]';
    if (ext === 'dex') return '[GEAR]';
    if (lower === 'resources.arsc') return '[BOX]';
    const m = {
        java: '[COFFEE]', kt: '[K]', smali: '[WRENCH]',
        xml: '[NEWS]', json: '[SCROLL]', yaml: '[SCROLL]', yml: '[SCROLL]', properties: '[SCROLL]',
        db: '[DB]', sqlite: '[DB]', sqlite3: '[DB]', realm: '[DB]',
        png: '[IMG]', jpg: '[IMG]', jpeg: '[IMG]', gif: '[IMG]', svg: '[IMG]', webp: '[IMG]', ico: '[IMG]',
        js: '[BOOK]', html: '[WEB]', css: '[PALETTE]',
        cer: '[LOCK]', pem: '[LOCK]', p12: '[LOCK]', rsa: '[LOCK]', dsa: '[LOCK]', ec: '[LOCK]', mf: '[SCROLL]', sf: '[SCROLL]',
        so: '[GEAR]',
    };
    return m[ext] || '[DOC]';
}
...
function populateQuickAccess(r) {
    ...
    if (r.files && r.files.indexOf('AndroidManifest.xml') >= 0) {
        items.push({ path: 'AndroidManifest.xml', name: 'Manifest', icon: '[LIST]', desc: 'AndroidManifest.xml' });
    }
    const dexFiles = (r.files || []).filter(f => /^classes\d*\.dex$/.test(f)).sort();
    for (const d of dexFiles.slice(0, 3)) {
        items.push({ path: d, name: d, icon: '[GEAR]', desc: 'DEX bytecode' });
    }
    if (r.files && r.files.indexOf('resources.arsc') >= 0) {
        items.push({ path: 'resources.arsc', name: 'resources.arsc', icon: '[BOX]', desc: 'Compiled resources' });
    }
    const cert = (r.files || []).find(f => /META-INF\/.+\.(RSA|DSA|EC)$/i.test(f));
    if (cert) items.push({ path: cert, name: 'Cert', icon: '[LOCK]', desc: 'Signing certificate' });
    ...
    const render = () => {
        ...
        viewer.innerHTML = '<div class="binary-viewer">' +
            ...
            '<div class="hex-controls">' +
                '<button class="hex-nav" data-action="first" ' + (cur === 0 ? 'disabled' : '') + '>[PREV]</button>' +
                '<button class="hex-nav" data-action="prev"  ' + (cur === 0 ? 'disabled' : '') + '>[LEFT]</button>' +
                '<span class="hex-page-info">Page ' + (cur + 1) + ' / ' + totalPages + ' · offset 0x' + off.toString(16) + '</span>' +
                '<button class="hex-nav" data-action="next" ' + (cur >= totalPages - 1 ? 'disabled' : '') + '>[PLAY]</button>' +
                '<button class="hex-nav" data-action="last" ' + (cur >= totalPages - 1 ? 'disabled' : '') + '>[NEXT]</button>' +
            '</div>' +
