(() => {
  ...
  async function renderReportTemplates(search = '') {
    ...
    try {
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px;">
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <button class="btn btn-ghost" onclick="exportReportTemplates()">[DOWN] Export JSON</button>
          <button class="btn btn-ghost" onclick="triggerImportReportTemplates()">[UP] Import JSON</button>
        </div>
        <button class="btn btn-primary" onclick="openReportTemplateModal()">[PLUS] Add Template</button>
      </div>
    `;
      ...
      <div class="domain-grid">
        ${templates.map((name) => `
          <div class="domain-card" onclick="openReportTemplateModalByName('${encodeURIComponent(name)}')">
            <div class="domain-name">[DOC] ${window.esc(name)}</div>
            ...
            <div style="margin-top: 16px; display: flex; gap: 8px;">
              <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation(); openReportTemplateModalByName('${encodeURIComponent(name)}')">[EDIT] Edit</button>
              <button class="btn btn-ghost btn-sm" style="color: var(--accent-red);" onclick="event.stopPropagation(); deleteReportTemplateByName('${encodeURIComponent(name)}')">[TRASH] Delete</button>
            </div>
          </div>
        `).join('')}
      </div>
    `;
      ...
      container.innerHTML = `<div class="error-state">[FAIL] ${window.esc(err.message)}</div>`;
    }
  }
  ...
  async function openReportTemplateModal(name = '') {
    ...
    contentInput.value = '';
    title.textContent = name ? '[LOG] Edit Template' : '[PLUS] New Template';
